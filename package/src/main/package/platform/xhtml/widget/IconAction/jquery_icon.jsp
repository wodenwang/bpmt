	...
	$(function() {
		...
		Core.fn($zone, 'initIcon', function() {
			...
			if (icon && icon.indexOf('.') > 0) {
				...
				var $span = $('<span style="margin-right: 10px;font-weight: bold;color:blue;">图标预览:</span>');
				var $button = $('<button text="false" type="button" icon="'+icon+'" title="'+icon+'">结果预览</button>');
				$resultZone.append($span);
				$resultZone.append($button);
				$button.styleButton();
			}
		});

		$('button[name=selectIcon]', $zone).click(function() {
			var $win = Ajax.win('${_acp}/jqueryIconList.shtml', {
				title : '请选择图标',
				minWidth : 800,
				maxHeight : 400,
			...
			Core.fn($win, 'callback', function(title) {
				var val = title;
				if (val.substring(1).indexOf('.') <= 0) {
					val = val.substring(9);
				}
				$text.val(val);
				$win.dialog("close");
...
<span name="resultZone" style="padding-right: 30px;"></span>
