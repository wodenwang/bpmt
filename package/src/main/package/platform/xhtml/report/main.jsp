...
<script type="text/javascript">
	$(function() {
		...
		//初始化模式
		{
			var querySize = new Number('${fn:length(config.table.querys)}');
...
<div tabs="true" max="10" id="${_zone}_tabs" main="true">
	...
	<div title="${wpf:lan(title)}">
		<form zone="${_zone}_list" action="${_acp}/list.shtml" query="true" id="${_zone}_list_form" method="get" option="{loading:true}">
			...
			<%--查询条件 --%>
			<c:if test="${fn:length(config.table.querys)>0}">
				<table class="ws-table" col="2">
					<c:forEach items="${config.table.querys}" var="vo">
					...
					<tr whole="true">
						<th class="ws-bar">
							...
							<div class="right ws-group">
								<button type="submit" icon="search" text="true" name="query">${wpf:lan("#:zh[查询]:en[Query]#")}</button>
							</div>
						</th>
					</tr>
				</table>
		...
		<div name="listZone" id="${_zone}_list">
