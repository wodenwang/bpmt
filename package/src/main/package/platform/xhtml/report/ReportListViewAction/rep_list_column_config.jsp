	...
	$(function() {
		...
		Core.fn($zone, 'storeTree', function() {
			...
			$.each(nodes, function(i, node) {
				...
				obj.sort = i;
				obj.id = node.id;
				obj.listSort = node.listSort ? node.listSort : 0;
				var $textarea = $('<textarea name="column.waitColumn"></textarea>');
		...
		Core.fn($zone, 'deleteColumn', function(treeNode) {
			if (treeNode.id) {// 编辑的情况下才需要
				var $textarea = $('<textarea name="column.deleteColumn"></textarea>');
				var json = {};
				json.id = treeNode.id;
		...
		Core.fn($zone, 'editColumn', function(treeNode) {
			...
			if ($target.size() > 0) {
				...
				if (treeNode._type == 'show') {
					...
					url = '${_acp}/columnLineConfigForm.shtml';
				}
...
<div style="overflow: auto; zoom: 1;">
	...
	<div style="margin-left: 310px; overflow: auto; zoom: 1;"
		id="${_zone}_main_zone"></div>
</div>
