	...
	$(function() {
		...
		Core.fn($zone, 'getCounts', function() {
			Ajax.json('${_acp}/getResourceCount.shtml', function(result) {
				...
				Ui.changeCurrentTitle('${_zone}_news_tab', '图文<font color="red">[' + result.news_count + ']</font>');
			}, {
		...
		Core.fn('${_zone}_news_tab', 'delete', function(mediaId) {
			Core.fn($zone, 'delete')($("#${_zone}_news_tab"), mediaId);
		});
		...
		$('button[name=upload]', $zone).click(function() {
			...
			Core.fn($win, 'refresh', function() {
				$win.dialog("close");
				Ajax.post($zone, '${_acp}/resource.shtml?mpKey=${param.mpKey}');
			});
		});
...
<div tabs="true" button="left">
	<div title="图片" id="${_zone}_image_tab" init="${_acp}/listResource.shtml?type=image&mpKey=${param.mpKey}"></div>
	<div title="语音" id="${_zone}_voice_tab" init="${_acp}/listResource.shtml?type=voice&mpKey=${param.mpKey}"></div>
	<div title="视频" id="${_zone}_video_tab" init="${_acp}/listResource.shtml?type=video&mpKey=${param.mpKey}"></div>
	<div title="图文" id="${_zone}_news_tab" init="${_acp}/listResource.shtml?type=news&mpKey=${param.mpKey}"></div>
</div>
