		...
		<c:when test="${type == 'image'}">
			...
			<c:forEach items="${dp.list}" var="vo">
				<tr>
					...
					<td class="left"><span tip="true" selector=".logo"><img src="${vo.url}" style="width: 30px; margin-bottom: -8px;" />${vo.fileName}<div class="logo">
								<img src="${vo.url}" style="width: 150px;" />
							</div></span></td>
					<td class=""><a href="${vo.url}" target="_blank">${vo.url}</a></td>
					<td class="right"><f:formatDate value="${vo.updateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
				</tr>
			</c:forEach>
		</c:when>
		<c:when test="${type == 'news'}">
			<tr>
				<th style="width: 20px;">操作</th>
				<th>资源ID</th>
				<th>内容简述</th>
		...
		<c:otherwise>
			...
			<c:forEach items="${dp.list}" var="vo">
				<tr>
					<td class="center"><button type="button" name="del" value="${vo.mediaId}" icon="trash" text="false">删除</button></td>
					<td class="left">${vo.mediaId}</td>
					<td class="left"><a href="javascript:void(0);" onclick="Ajax.download('${_acp}/downloadResource.shtml?mpKey=${param.mpKey}&mediaType=${type}&mediaId=${vo.mediaId}&fileName=${vo.fileName}');">${vo.fileName}</a></td>
					<td class="right"><f:formatDate value="${vo.updateTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
				</tr>
			</c:forEach>
		</c:otherwise>
