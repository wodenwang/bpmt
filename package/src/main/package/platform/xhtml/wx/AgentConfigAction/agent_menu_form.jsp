...
<table class="ws-table">
	...
	<c:if test="${menuType!=0}">
		<tr>
			<th>菜单类型</th>
			<td><input name="${pixel}.menuType" value="99" type="radio" /><label>事件绑定</label><input name="${pixel}.menuType" value="1" type="radio" /><label>视图绑定</label><input name="${pixel}.menuType" value="2" type="radio" /><label>自定义网址</label></td>
		</tr>
		<tr menuType="1">
			<th>视图URL</th>
		...
		<tr menuType="99">
			<th>事件处理器</th>
			<td><wcm:widget name="${pixel}.commandKey" cmd="wxcommand[qy;MENU]" value="${vo.commandKey}" /></td>
		</tr>
		<tr menuType="99">
			<th>动态入参(脚本类型)</th>
			<td><wcm:widget name="${pixel}.paramType" cmd="select[@com.riversoft.platform.script.ScriptTypes]" value="${vo.paramType}"></wcm:widget></td>
