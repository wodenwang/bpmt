		...
		<table class="ws-table">
			...
			<c:forEach items="${fields}" var="field">
				<tr>
					...
					<td colindex="field_show"><c:if test="${field.pri!=null&&!field.pri.scriptOnly}">
							<input type="checkbox" value="${field.pri.priKey}" name="priKey" checkstate="${wcm:contains(priKeys,field.pri.priKey)}" />
						</c:if></td>
					<td class="right"><c:if test="${field.pri!=null}">
							<wcm:widget name="relate" cmd="prigroup[${groupId};${field.pri.priKey}]" />
						</c:if></td>
				</tr>
			</c:forEach>
		</table>
