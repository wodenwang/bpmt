...
<script type="text/javascript">
	$(function() {
		...
		Core.fn('${_zone}_list', 'create', function() {
			var $tab = Ui.openTab('创建任务', '${_acp}/createZone.shtml');
			Core.fn($tab, 'submitForm', Core.fn($zone, 'submitForm'));
		});

		Core.fn('${_zone}_list', 'del', function(key) {
			Ui.confirm('确认删除任务?', function() {
				Ajax.post('${_zone}_msg', '${_acp}/delete.shtml?jobKey=' + key, {
		...
		Core.fn('${_zone}_list', 'edit', function(key) {
			var $tab = Ui.openTab('编辑[' + key + ']结构', '${_acp}/updateZone.shtml?jobKey=' + key);
			Core.fn($tab, 'submitForm', Core.fn($zone, 'submitForm'));
		});

		//改变状态
		Core.fn('${_zone}_list', 'active', function(key, flag) {
			Ajax.post('${_zone}_msg', '${_acp}/active.shtml', {
				data : {
					jobKey : key,
					activeFlag : flag ? 1 : 0
				},
	...
	<div title="计划任务配置">
		<form zone="${_zone}_list" action="${_acp}/list.shtml" query="true"
			...
			<table class="ws-table">
				<tr>
					<th>主键(精确)</th>
					<td><wcm:widget name="_se_jobKey" cmd="text">不支持命令</wcm:widget></td>
					<th>主键(模糊)</th>
					<td><wcm:widget name="_sl_jobKey" cmd="text">不支持命令</wcm:widget></td>
				</tr>
				<tr>
					<th>描述(精确)</th>
