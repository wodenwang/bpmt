...
<script type="text/javascript">
	$(function() {
		...
		//接收邮件
		$('button[name=receive]', $zone).click(function() {
			Ajax.post("${_zone}_msg", '${_acp}/receiveMail.shtml', {
...
<form zone="${_zone}_list" action="${_acp}/listInbox.shtml" query="true" id="${_zone}_list_form" method="get">
	<input name="_field" type="hidden" value="RECEIVE_DATE" /> <input name="_dir" type="hidden" value="desc" />
	<table class="ws-table">
		...
		<tr>
			<th>发件人</th>
			<td><wcm:widget name="_sl_FROM_ADDR" cmd="text" /></td>
			<th>抄送人</th>
			<td><wcm:widget name="_sl_CC_ADDRS" cmd="text" /></td>
		</tr>
		<tr>
			<th>接收时间(>=)</th>
			<td><wcm:widget name="_dnl_RECEIVE_DATE" cmd="date[datetime]" /></td>
			<th>接收时间(<=)</th>
			<td><wcm:widget name="_dnm_RECEIVE_DATE" cmd="date[datetime]" /></td>
		</tr>
		<tr>
			<th class="ws-bar">
				<div class="left ws-group">
					<button type="button" icon="mail-closed" name="receive">收邮件</button>
...
<form id="${_zone}_delete_form" action="${_acp}/removeReceiveMail.shtml" sync="true">
	<%--查询结果 --%>
