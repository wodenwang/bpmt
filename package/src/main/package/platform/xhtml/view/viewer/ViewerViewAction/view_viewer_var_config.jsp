...
<script type="text/javascript">
	$(function() {
		...
		$('button[name=varsAdd]', $zone).click(function() {
			var $tabs = $('#${_zone}_vars_tabs');
			Ajax.tab($tabs, '${_acp}/varsForm.shtml', {
				data : {
					type : 'vars',
					pixel : 'vars.B' + Core.nextSeq()
				}
			});
		});
	});
...
<div class="ws-bar">
	<div class="ws-group left">
		<button icon="plus" type="button" name="varsAdd" title="展示变量允许在界面展示中使用,包括展示字段,表单字段等." tip="true">新增变量</button>
	</div>
</div>
<div tabs="true" button="left" sort="y" id="${_zone}_vars_tabs">
	<c:if test="${vars!=null&&fn:length(vars)>0}">
		<c:forEach items="${vars}" var="vo" varStatus="status">
			<div title="${vo.var}" close="true">
				<c:set var="pixel" value="vars.A${status.index}" />
