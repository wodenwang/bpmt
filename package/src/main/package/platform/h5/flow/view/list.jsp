...
<header data-am-widget="header" class="am-header am-header-default">
	<div class="am-header-left am-header-nav">
		<c:if test="${wpf:check(config.createBtn.pri)}">
			<a href="${_acp}/form.shtml?_params=${wcm:urlEncode(param._params)}&ordFlag=1" class="" id="${_zone}_btn_add"><i class="am-header-icon am-icon-plus-circle"></i></a>
		</c:if>
	</div>
		...
		<ul class="am-list">
			<c:forEach items="${dp.list}" var="vo">
				...
				<c:choose>
					...
					<c:when test="${config.table.weixin.listMode==1}">
						<li class="am-g am-list-item-desced am-list-item-thumbed am-list-item-thumb-left">
							<div class="am-u-sm-3 am-list-thumb">
								<a href="${_acp}/${clickUrl}.shtml?_params=${wcm:urlEncode(param._params)}&_ORD_ID=${vo.ORD_ID}&ordFlag=1" class=""><wpf:image
										value="${wpf:script(config.table.weixin.imgType,config.table.weixin.imgScript,context)}" /></a>
							</div>
							<div class=" am-u-sm-9 am-list-main">
								<h3 class="am-list-item-hd">
									<a href="${_acp}/${clickUrl}.shtml?_params=${wcm:urlEncode(param._params)}" class=""><wpf:script script="${config.table.weixin.titleScript}" type="${config.table.weixin.titleType}"
											context="${context}" /></a>
								</h3>
					...
					<%-- 纯文模式 --%>
					<c:otherwise>
						<li class="am-g am-list-item-dated"><a href="${_acp}/${clickUrl}.shtml?_params=${wcm:urlEncode(param._params)}&_ORD_ID=${vo.ORD_ID}&ordFlag=1" class="am-list-item-hd"><wpf:script
