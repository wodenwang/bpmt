	...
	public static String pattern(String code, Object... objs) {
		...
		while (matcher.find()) {
			...
			case "now":// 时间模式
				...
				if (p == null) {
					throw new SystemRuntimeException(ExceptionType.BUSINESS, "无法为{now}获取参数.");
				}
			...
			case "req":// 表单入参
				...
				if (name == null) {
					throw new SystemRuntimeException(ExceptionType.BUSINESS, "无法为{req}获取参数.");
				}
			...
			case "seq":// 自动序列号
				...
				if (tableName == null || columnName == null || size == null || size < 1) {
					throw new SystemRuntimeException(ExceptionType.BUSINESS, "无法为{seq}获取足够数量的参数.");
				}

				Map<String, Object> o = JdbcService.getInstance().findSQL(
						"select max(" + columnName + ") maxColumn from " + tableName + " where " + columnName
								+ " like ?", buff.toString() + "%");
				int seq = 0;
