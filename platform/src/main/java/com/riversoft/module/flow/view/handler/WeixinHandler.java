...
public class WeixinHandler implements Handler {
	...
	public void handle(DataPO tablePO) {
		...
		DataPO weixinPO;
		Map<String, Object> po = (Map<String, Object>) ORMService.getInstance().findByPk("VwFlowBasicWeixin", viewKey);
		if (po != null) {
			weixinPO = new DataPO("VwFlowBasicWeixin", po);
		} else {
			weixinPO = new DataPO("VwFlowBasicWeixin");
