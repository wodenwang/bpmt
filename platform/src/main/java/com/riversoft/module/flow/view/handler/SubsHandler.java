...
public class SubsHandler implements Handler {
	...
	public void handle(DataPO tablePO) {
		...
		if (subNames != null && subNames.length > 0) {
			for (String pixel : subNames) {
				...
				if (StringUtils.isEmpty(subKey)) {
					subKey = IDGenerator.next();
				}

				if (StringUtils.isNotEmpty(name)) {// 系统内置标签
					DataPO subPO = new DataPO("VwFlowBasicSubSys");
					...
					subPO.set("pri", pri);
					sysSubs.add(subPO.toEntity());
				} else if (StringUtils.isNotEmpty(action)) {// 视图标签
					DataPO subPO = new DataPO("VwFlowBasicSubView");
					subPO.set("subKey", subKey);
					subPO.set("viewKey", tablePO.getString("viewKey"));
		...
		ORMService.getInstance().saveBatch(sysSubs);
		ORMService.getInstance().saveBatch(viewSubs);

		tablePO.set("sysSubs", null);
