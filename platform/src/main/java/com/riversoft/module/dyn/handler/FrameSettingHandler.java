...
public class FrameSettingHandler implements Handler {
	...
	private boolean handleListSort(DataPO tablePO) {
		...
		// 先重置listSort
		ORMService.getInstance().executeHQL("update VwDynColumn set listSort = -1 where viewKey = ?",
				tablePO.getString("viewKey"));
		ORMService.getInstance().executeHQL("update VwDynColumnShow set listSort = -1 where viewKey = ?",
				tablePO.getString("viewKey"));
