...
public class WeixinHandler implements Handler {
	...
	public void handle(DataPO tablePO) {
		...
		DataPO weixinPO;
		Map<String, Object> po = (Map<String, Object>) ORMService.getInstance().findByPk("VwDynWeixin", viewKey);
		if (po != null) {
			weixinPO = new DataPO("VwDynWeixin", po);
		} else {
			weixinPO = new DataPO("VwDynWeixin");
