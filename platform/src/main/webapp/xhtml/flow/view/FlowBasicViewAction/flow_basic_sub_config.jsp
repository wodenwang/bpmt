		...
		<c:forEach items="${subs}" var="vo" varStatus="status">
			<c:choose>
				...
				<c:when test="${vo.name!=null}">
					<div title="${vo.busiName}">
						<c:set var="pixel" value="subs.A${status.index}" />
						<input type="hidden" name="subs" value="${pixel}" /> <input type="hidden" name="${pixel}.subKey" value="${vo.subKey}" />
						<table class="ws-table">
							<tr>
								<th>系统名</th>
								<td><input type="hidden" name="${pixel}.name" value="${vo.name}" />${vo.name}</td>
							</tr>
							<tr>
								<th>展示名</th>
